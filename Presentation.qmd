...
-   Say, given \textcolor{red}{any sample} $Y = (y_{1},...,y_{n})$ the posterior \$ P(\theta \| y) \$ looks approximately like
...
## Set identification {.smaller}

-   A parameter set $\Theta$ is said to be (point) identified if $\Theta = \{\theta\}$

-   A parameter set $\Theta$ is said to be set identified if $\exists \theta_{0}, \theta_{1} \in \Theta s.t. \theta_{0} \neq \theta_{1}$
...
-   Note this is for any draw of the data and the priors must be equal. Not necessarily the posteriors
...
### Examples of the problem {.smaller}
Or
```{r, echo = F}
...
## The solution: Posterior upper and lower probabilities (Theorem 1)

-   We can define $\Pi_{\theta | Y} = \{\pi_{\theta|Y} = \int_{\Phi}\pi_{\theta|\phi}d\pi_{\phi |Y}\}$
-   For any set $\Delta$ we can define the posterior lower probability as $$
...
-   $\underline{\pi}_{\theta|Y}$ is the lowest possible posterior probability assigned the event $\{\theta \in D\}$ no matter which prior is chosen by the researcher ::: notes The assumptions that go into this theorem are basically that the probability distributions are well defined and that the maps are lower semi-continuous :::

### Intuition for the interpretation
...
## Set of posterior means

-   Theorem 1 gives us lower and upper probabilities for an arbitrary event $D$, next step is to summarise the information without specifying $D$.
-   The paper proposes the set of posterior means $\{E_{\theta | Y} | \pi_{\theta | Y} \in \Pi_{\theta | Y}\}$.
-   Theorem 2: Under measurability conditions and $E_{\phi | Y}[\sup_{\theta \in IS_{\theta}(\phi)}||\theta||] < \infty$, given that $co(IS_{\theta}(\phi))$ is the convex hull of $IS_{\theta}(\phi)$ and $E^{A}_{\phi|Y}$ is the aumann expectation with probability measure $\phi_{\phi |Y}$ we have $$
    \{E_{\theta | Y} | \pi_{\theta | Y} \in \Pi_{\theta | Y}\} = E^{A}_{\phi|Y}[co(IS_{\theta}(\phi))]
    $$

### What is the Aumann expectation and what is the expectation of a random set?
...
### A little note on support functions
...
- In the context of this paper, they will come up a lot.
### Why the Aumann expectation and why the set of posterior means?
...
\end{align*}
$$
- Note: They claim that this can be calculated by Monte Carlo, how easy that would be I do not know.
- In a more general setting for for $A  := IS_{\theta}(\phi)$ given $\sigma_{A}(q) : \mathcal{P}(\Theta) \times \mathbb{R}^{n} \to \mathbb{R} \cup \{+\infty\}$, we have $\sigma_{E^{A}_{\phi|Y}[co(IS_{\theta}(\phi))]} = E_{\phi |Y}[\sigma_{IS_{\theta}(\phi)}]$
...
-- i) Posterior of $\phi$ is consistent for $\phi_{0}$, $P_{Y^{\infty}|\theta_{0}}$-a.s.
...
### Coverage of robust credible region
- Asumption 3:
