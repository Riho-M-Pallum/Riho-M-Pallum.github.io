...
- Indeed the first problem is that the posteriors can be different depending on the draw, even though the priors are the same
...
### The problem with credible intervals in set identified models {.smaller}

Asymptotically, the credible interval concentrates in the identified set, therefore for some parameter values the credible intervals never contain the true value. Rather it reflects priors. 
```{r, echo = F}
...
data <- data.frame(x = x_values, y = combined_distribution, bell_curve = bell_curve, cdf = cdf)
...
$$
Ay_{t} = \epsilon_{t}, \quad y_{t}\in \mathbb{R}^{2}, A \in \mathcal{M}(2,2), \epsilon_{t} \sim \mathcal{N}(0, I_{2})
$$
- The reduced form VAR(0) (what we can observe and estimate) is $y_{t} = A^{-1}\epsilon_{t} = \eta_{t}$ with $E[\eta_{t}\eta_{t}^{T}]= \Sigma =  A^{-1}(A^{-1})^{T} = (A^{T}A)^{-1}$
-   Note that $\forall Q \in \mathcal{O}(2)$, we have that $\Sigma = (A^{T}QQ^{T}A)^{-1}$, the model is set identified
- We will impose restrictions on $A$ later (this is what makes it a SVAR)
...
$$
y_{t} = A^{-1}\epsilon_{t}    = \eta_{t} \quad E[\eta_{t}\eta_{t}^{T}]= \Sigma = (A^{T}QQ^{T}A)^{-1}  \forall Q \in \mathcal{O}(2)
$$
